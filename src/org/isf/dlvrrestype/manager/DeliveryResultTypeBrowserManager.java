...
public class DeliveryResultTypeBrowserManager {
	...
	public boolean deleteDeliveryResultType(DeliveryResultType deliveryresultType) {
		try {
			return ioOperations.deleteDeliveryResultType(deliveryresultType.getCode());
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return false;
		}
	 ...
	 * @return <code>true</code> if the delivery result type has been deleted, <code>false</code> otherwise.
	 */
	public boolean deleteDeliveryResultType(String code) {
		try {
			return ioOperations.deleteDeliveryResultType(code);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return false;
		}
