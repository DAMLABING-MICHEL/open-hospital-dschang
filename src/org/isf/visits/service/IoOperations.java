...
public class IoOperations {
	...
	public int newVisit(Visit visit) throws OHException {
		...
		int visitID = 0;
		try {
			String query = "INSERT INTO VISITS (VST_ID, VST_PAT_ID, VST_DATE, VST_NOTE, VST_SMS, VST_CREATE_BY, VST_CREATE_DATE) VALUES (?, ?, ?, ?, ?, ?, ?)";
			parameters.add(visit.getVisitID());
			parameters.add(visit.getPatID());
			parameters.add(visit.getVisitDate().getTime());
	...
	private Visit toVisit(ResultSet resultSet) throws SQLException {
		...
		visit.getVisitDate().setTime(resultSet.getTimestamp("VST_DATE"));
