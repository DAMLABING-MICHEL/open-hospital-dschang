...
public class DicomViewGui extends JPanel {
	...
	public DicomViewGui(Patient patient, String serieNumber) {
		...
		if (patID >= 0)
			frames = DicomManagerFactory.getManager().getSerieDetail(patID,
					serieNumber);
	...
	private void initComponent() {
		...
		if (patID <= 0) {
			...
			jPanelCenter.add(composeCenter(jPanelCenter.getWidth(),
					jPanelCenter.getHeight(), true));
	...
	private void reInitComponent() {
		if (patID <= 0) {
			...
			// BufferedImage immagineResized = Scalr.resize(tmpImg,
			// Math.round(value));
	...
	private JPanel composeCenter(int w, int h, boolean calculate) {
		...
		drawPatientUpRight(canvas, imageCanvas.getWidth(),
				imageCanvas.getHeight());
		drawInfoFrameBottomLeft(canvas, imageCanvas.getWidth(),
				imageCanvas.getHeight());
		drawStudyUpRight(canvas, imageCanvas.getWidth(),
				imageCanvas.getHeight());
		drawSerieBottomRight(canvas, imageCanvas.getWidth(),
				imageCanvas.getHeight());
	...
	private void drawPatientUpRight(Graphics2D canvas, int w, int h) {
		...
		String txt = "";
		canvas.drawString(
				MessageBundle.getMessage("angal.dicom.image.patient.oh"), 10,
				hi);
		hi += VGAP;
		txt = ohPatient.getName();
		canvas.drawString(
				MessageBundle.getMessage("angal.dicom.image.patient.name")
						+ " : " + txt, 10, hi);
		hi += VGAP;
		txt = "" + ohPatient.getAge();
		canvas.drawString(
				MessageBundle.getMessage("angal.dicom.image.patient.age")
						+ " : " + txt, 10, hi);
		hi += VGAP;
		txt = "" + ohPatient.getSex();
		canvas.drawString(
				MessageBundle.getMessage("angal.dicom.image.patient.sex")
						+ " : " + txt, 10, hi);
		hi += VGAP;
		hi += VGAP;
		canvas.drawString(
				MessageBundle.getMessage("angal.dicom.image.patient.dicom"),
				10, hi);
		hi += VGAP;
		...
		canvas.drawString(
			...
			hi += VGAP;
			Image photo;
			 String fileName=PatientBrowserManager.getPatientPhotoPath(ohPatient.getCode());
			try {
				File fPhoto = new File(Param.string("PHOTOSDIR") + File.separator
						+ fileName);
				if (fPhoto.exists()) {
					photo = ImageIO.read(fPhoto);
				} else {

					photo = ImageIO.read(new File("rsc/images/nophoto.png"));

				}
				BufferedImage bi = new BufferedImage(photo.getWidth(this),
						photo.getHeight(this), BufferedImage.TYPE_INT_ARGB);
				bi.getGraphics().drawImage(photo, 0, 0, this);
				drawQuadrant(bi.getGraphics(), photo.getHeight(this),
						photo.getWidth(this), Color.WHITE);
	...
	private void drawInfoFrameBottomLeft(Graphics2D canvas, int w, int h) {
		...
		canvas.drawString(MessageBundle.getMessage("angal.dicom.image.zoom")
				+ " : " + txt, 10, hi);
		hi -= VGAP;
		txt = "[" + (frameIndex + 1) + "]/" + frames.length;
		canvas.drawString(MessageBundle.getMessage("angal.dicom.image.frames")
				+ " : " + txt, 10, hi);
		canvas.setColor(orig);
	}
	...
	private void drawStudyUpRight(Graphics2D canvas, int w, int h) {
		...
		if (txt == null)
			txt = "";
		canvas.drawString(MessageBundle.getMessage("angal.dicom.image.studyid")
				+ " : " + txt, ws, hi);
		...
		if (d != null)
			...
			txt = "";
		canvas.drawString(MessageBundle.getMessage("angal.common.date") + " : "
				+ txt, ws, hi);
		canvas.setColor(orig);
	}
	...
	private void drawSerieBottomRight(Graphics2D canvas, int w, int h) {
		...
		if (txt == null)
			txt = "";
		canvas.drawString(MessageBundle.getMessage("angal.dicom.image.serie.n")
				+ " " + txt, ws, hi);
		canvas.setColor(orig);
	}
	...
	private void refreshFrame() {
		...
		tmpDbFile = DicomManagerFactory.getManager().loadDettaglio(id, patID,
				serieNumber);
	...
	private void getImageFromDicom(FileDicomDetail dett) {
		try {
			...
			ImageInputStream imageInputStream = ImageIO
					.createImageInputStream(dett.getDicomData()
							.getBinaryStream());
			...
			try {
				...
				JOptionPane.showMessageDialog(null,
						MessageBundle.getMessage("angal.dicom.load.err")
								+ " : " + dett.getFileName(),
						MessageBundle.getMessage("angal.dicom.err"),
