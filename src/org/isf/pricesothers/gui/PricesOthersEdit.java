...
public class PricesOthersEdit extends JDialog {
		...
		if (jButtonOK == null) {
			...
			jButtonOK.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent event) {
					...
					pOther.setAccount(jTextFieldAccount.getText().trim());
	...
	private JCheckBox getJCheckBoxUndefined() {
		if (jCheckBoxUndefined == null) {
			...
			jCheckBoxUndefined.setText(MessageBundle.getMessage("angal.pricesothers.undefined")); 
	...
	private JCheckBox getJCheckBoxDischarge() {
		if (jCheckBoxDischarge == null) {
			...
			jCheckBoxDischarge.setText(MessageBundle.getMessage("angal.pricesothers.discharge1"));
	...
	private JCheckBox getJCheckBoxDaily() {
		if (jCheckBoxDaily == null) {
			...
			jCheckBoxDaily.setText(MessageBundle.getMessage("angal.pricesothers.daily"));//
	...
	private JTextField getJTextFieldCode() {
		...
		if(!insert) jTextFieldCode.setEditable(false);
	...
	private JPanel getJPanelData() {
		if (jPanelData == null) {
			...
			jPanelData.add(getJPanelAccount());
			jPanelData.add(getJPanelParameters());
		}
		return jPanelData;
	}
	private JPanel getJPanelAccount() {
		if (jPanelAccount == null) {
			jPanelAccount = new JPanel();
			jPanelAccount.setLayout(new GridLayout(2, 2));
			jPanelAccount.add(getJLabelAccount());
			jPanelAccount.add(getJTextFieldAccount());
		}
		return jPanelAccount;
	}
	private JLabel getJLabelAccount() {
		if (jLabelAccount == null) {
			jLabelAccount = new JLabel(MessageBundle.getMessage("angal.medicals.accountnumber"));
		}
		return jLabelAccount;
	}
	...
	private JTextField getJTextFieldAccount(){
		if (jTextFieldAccount == null) {
			...
			jTextFieldAccount.setText(insert? "" : pOther.getAccount());
		}
