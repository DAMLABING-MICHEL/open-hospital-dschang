...
public class DeathReasonBrowserEdit extends JDialog {
	...
	private EventListenerList deathReasonListeners = new EventListenerList();
    ...
    public void addDeathReasonListener(DeathReasonListener l) {
    	deathReasonListeners.add(DeathReasonListener.class, l);
    }

    public void removeDeathReasonListener(DeathReasonListener listener) {
    	deathReasonListeners.remove(DeathReasonListener.class, listener);
    }

    private void fireDeathReasonInserted() {
        AWTEvent event = new AWTEvent(new Object(), AWTEvent.RESERVED_ID_MAX + 1) {
        	private static final long serialVersionUID = 1L;
		};

        EventListener[] listeners = deathReasonListeners.getListeners(DeathReasonListener.class);
        for (int i = 0; i < listeners.length; i++)
            ((DeathReasonListener)listeners[i]).deathReasonInserted(event);
    }
    private void fireDeathReasonUpdated() {
        AWTEvent event = new AWTEvent(new Object(), AWTEvent.RESERVED_ID_MAX + 1) {
			private static final long serialVersionUID = 1L;
		};
        ...
        EventListener[] listeners = deathReasonListeners.getListeners(DeathReasonListener.class);
        for (int i = 0; i < listeners.length; i++)
            ((DeathReasonListener)listeners[i]).deathReasonUpdated(event);
    }
	 ...
	 */
	public DeathReasonBrowserEdit(JFrame owner, DeathReason old, boolean inserting) {
		super(owner,true);
		insert = inserting;
		deathReason = old;
		lastdescription= deathReason.getDescription();
		initialize();
	}
	...
	private void initialize() {
		...
		if (insert) {
			this.setTitle(MessageBundle.getMessage("angal.deathreason.newdeathreasonrecord"));
		} else {
			this.setTitle(MessageBundle.getMessage("angal.deathreason.editingdeathreason"));
		}
			...
			okButton.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (key.length()>5){
						JOptionPane.showMessageDialog(				
								null,
								MessageBundle.getMessage("angal.deathreason.codetoolongmaxchars"),
								MessageBundle.getMessage("angal.hospital"),
								JOptionPane.PLAIN_MESSAGE);
					...
					deathReason.setDescription(descriptionTextField.getText());
					deathReason.setCode(codeTextField.getText());
					boolean result = false;
					if (insert) {      // inserting
						result = manager.newDeathReason(deathReason);
						if (result) {
                           fireDeathReasonInserted();
                        }
                    	...
                    	if (descriptionTextField.getText().equals(lastdescription)){
    						...
    						//System.out.println("ID DEATH REASON............."+deathReason.getId());
    						result = manager.updateDeathReason(deathReason);
							if (result) {
								fireDeathReasonUpdated();
	                        } 
							else {
								JOptionPane.showMessageDialog(null, MessageBundle.getMessage("angal.distype.thedatacouldnotbesaved"));
	...
	private JTextField getDescriptionTextField() {
		if (descriptionTextField == null) {
			...
			if (!insert) {
				descriptionTextField.setText(deathReason.getDescription());
				lastdescription=deathReason.getDescription();
			} 
		}
	...
	private JTextField getCodeTextField() {
		if (codeTextField == null) {
			...
			if (!insert) {
				codeTextField.setText(deathReason.getCode());
	...
	private JLabel getJCodeLabel() {
		if (jCodeLabel == null) {
			...
			jCodeLabel.setText(MessageBundle.getMessage("angal.deathreason.codemaxchars"));
		}
		return jCodeLabel;
	}
