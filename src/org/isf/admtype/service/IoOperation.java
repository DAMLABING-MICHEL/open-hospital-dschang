...
public class IoOperation {
	...
	/**
	 * Returns and {@link AdmissionType} by its code.
	 * @return an admission type.
	 * @throws OHException if an error occurs.
	 */
	public AdmissionType getAdmissionType(String code) throws OHException {
		AdmissionType admissionType = null;
		String query = "select * from ADMISSIONTYPE WHERE ADMT_ID_A = ?";
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> params = Collections.<Object> singletonList(code);
		try{
			ResultSet resultSet = dbQuery.getDataWithParams(query, params, true);
			if (resultSet.next()) {
				admissionType=new AdmissionType(resultSet.getString("ADMT_ID_A"), resultSet.getString("ADMT_DESC"));
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} finally{
