...
public class AdmissionTypeBrowserManager {
	...
	/**
	 * Returns an {@link AdmissionType} by it code.
	 * @return an admission types or <code>null</code> if there is no admission type for the provided code.
	 */
	public AdmissionType getAdmissionType(String code) {
		try {
			return ioOperations.getAdmissionType(code);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
		}
