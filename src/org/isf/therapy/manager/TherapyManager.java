...
public class TherapyManager {
	...
	public Therapy createTherapy(TherapyRow th) {
		return createTherapy(th.getTherapyID(), th.getPatID(), th.getMedical(), th.getQty(),
				...
				th.getNote(), th.isNotify(), th.isSms(), th.getQtyBougth());
	}
	...
	public Therapy createTherapy(int therapyID, int patID, Medical med, Double qty,
			...
			int freqInDay, String note, boolean notify, boolean sms, Double qtyBougth) {
		
		ArrayList<GregorianCalendar> datesArray = new ArrayList<GregorianCalendar>();
		
		GregorianCalendar stepDate = TimeTools.getServerDateTime();
		...
		Therapy th = new Therapy(therapyID,	patID, dates, med, qty, "", freqInDay, note, notify, sms, qtyBougth);
		datesArray.clear();
		dates = null;
	...
	public Therapy createTherapy(int therapyID, int patID, int medID, Double qty,
			GregorianCalendar startDate, GregorianCalendar endDate, int freqInPeriod,
			int freqInDay, String note, boolean notify, boolean sms, Double qtyBougth) {
	...
	/**
	 * Check weather the patient has therapies rows not yet bought.
	 * @param code
	 * @return
	 */
	public boolean hasTherapiesRowsNotYetBought(int code){
		try {
			Integer nbThRow =ioOperations.getTherapyRowsNotBougthCount(code);
			return nbThRow>0;
	...
	/**
	 * Update the qty of medical bougth from the therapy row
	 * @param thID
	 * @param qty
	 * @return
	 */
	public boolean updateBougthQuantity(int thID, Double qty){
		try {
			return ioOperations.updateQtyBougth( thID, qty);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return false;
		}
	 ...
	 */
	public boolean deleteAllTherapies(Integer patId) {
		try {
			return ioOperations.deleteAllTherapies(patId);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return false;
		}
