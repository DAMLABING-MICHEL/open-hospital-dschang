...
//	private JButton reportButton; TODO to enable when a report will be designed
	...
	public TherapyEdit(JDialog owner, Patient patient, boolean admitted) {
		super(owner, true);
		this.patient = patient;
		this.admitted = admitted;
		initComponents();
	...
	private void showVisits() {
		for (Visit visit : visits) {
			// System.out.println(th.numTherapy+"-"+formatDate(gc));
			if (visit.getVisitDate().get(GregorianCalendar.YEAR) == yearChooser.getYear()) {
				if (visit.getVisitDate().get(GregorianCalendar.MONTH) == monthChooser
						.getMonth()) {
					// jAgenda.addElement(index, th,
					// gc.get(GregorianCalendar.DAY_OF_MONTH));
					jAgenda.addElement(visit, visit.getVisitDate().get(GregorianCalendar.DAY_OF_MONTH));
				}
			}
		}
	}
	...
	private JCheckBox getSmsCheckBox() {
		if (smsCheckBox == null) {
			...
			if (!Param.bool("SMSENABLED") || admitted) {
				smsenable = false;
			} else {
				smsenable = true;
			}
		...
		if (saveButton == null) {
			...
			saveButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					String resultCheck = PatientBrowserManager.checkPatientInformation(patient, validationType);
					if(resultCheck.length()>0){
						JOptionPane.showMessageDialog(null,MessageBundle.getMessage("angal.patient.missinginformation")+"\n"+resultCheck);
						if(Param.bool("PATIENTEXTENDED")){
							PatientInsertExtended newrecord = new PatientInsertExtended(TherapyEdit.this, patient, false);
							newrecord.setValidationGroup(ValidationPatientGroup.GLOBAL);
							//newrecord.addPatientListener(TherapyEdit.this);
							newrecord.setVisible(true);
						}else{
							PatientInsert newrecord = new PatientInsert(TherapyEdit.this, patient, false);
							newrecord.setValidationGroup(ValidationPatientGroup.GLOBAL);
							//newrecord.addPatientListener(TherapyEdit.this);
							newrecord.setVisible(true);
						}
