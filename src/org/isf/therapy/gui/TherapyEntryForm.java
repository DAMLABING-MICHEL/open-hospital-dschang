...
public class TherapyEntryForm extends JDialog {
	...
	private JPanel getSearchPanel() {
		if (serachPanel == null) {
			...
			serachPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
			serachPanel.setLayout(new BoxLayout(serachPanel,
					BoxLayout.X_AXIS));
			serachPanel.add(getMedicalCode());
			serachPanel.add(getMedicalButton());
		}
		return serachPanel;
	}
	...
	private JPanel getMedicalsPanel() {
		if (medicalsPanel == null) {
			...
			medicalsPanel.setBorder(new TitledBorder(null, MessageBundle.getMessage("angal.therapy.pharmaceutical"), //$NON-NLS-1$
					TitledBorder.LEADING, TitledBorder.TOP, null, null));
			//
			medicalsPanel.add(getSearchPanel());
			///
...
/////////////////////////////////////////////////////
	private JTextField getMedicalCode() {
		...
		medicalCode.setPreferredSize(new Dimension(100, 25));
		...
		medicalCode.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_ENTER || e.getKeyCode() == KeyEvent.VK_TAB
					...
					if(text.equals("")){
						medArray = medBrowser.getMedicals();
						for (Medical aMedical : medArray) {
							model.addElement(aMedical);
						}
						medicalsList.setModel(model);
						medicalsList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	...
	private JButton getMedicalButton() {
		...
		medicalButton.setMaximumSize(new Dimension(50, 25));
		medicalButton.setMinimumSize(new Dimension(30, 25));
		medicalButton.setPreferredSize(new Dimension(40, 25));
	...
	private JDateChooser getStartDate() {
		if (therapyStartdate == null) {
			...
			therapyStartdate.setLocale(new Locale(Param.string("LANGUAGE")));
		...
		if (buttonOK == null) {
			...
			buttonOK.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					boolean sms = false;
					Double qtyBougth=0.0;
					
					thRow = new TherapyRow(therapyID, patID, startDate, endDate, medical, qty, unitID, freqInDay, freqInPeriod, note, notify, sms, qtyBougth);
