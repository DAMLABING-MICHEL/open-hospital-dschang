...
public class PatientSummary {
	...
	public JPanel getPatientDataPanel() {
		...
		dataPanel.add(setMyBorder(getPatientNamePanel(),
				MessageBundle.getMessage("angal.admission.namem")));
		dataPanel.add(setMyBorder(getPatientTaxCodePanel(),
				MessageBundle.getMessage("angal.admission.taxcode")));
		dataPanel.add(setMyBorder(getPatientSexPanel(),
				MessageBundle.getMessage("angal.admission.sexm")));
		dataPanel.add(setMyBorder(getPatientAgePanel(),
				MessageBundle.getMessage("angal.admission.agem")));
		dataPanel.add(setMyBorder(getPatientNotePanel(),
				MessageBundle.getMessage("angal.admission.patientnotes")));
	...
	public JPanel getPatientCompleteSummary() {
		...
		dataPanel.add(setMyBorder(getPatientParentNewsPanel(),
				MessageBundle.getMessage("angal.admission.parents")));
		
		dataPanel.add(getPatientKinAndTelephonePanel());
		dataPanel.add(getPatientBloodAndEcoPanel());

		p.add(dataPanel, BorderLayout.CENTER);
		p.add(setMyBorder(getPatientNotePanel(),
				MessageBundle.getMessage("angal.admission.patientnotes")),
				BorderLayout.SOUTH);
	...
	private JPanel getPatientAddressAndCityPanel() {
		...
		dataPanel.add(setMyBorder(getPatientAddressPanel(),
				MessageBundle.getMessage("angal.admission.addressm")));
		dataPanel.add(setMyBorder(getPatientCityPanel(),
				MessageBundle.getMessage("angal.admission.city")));
		return dataPanel;
	}
	...
	private JPanel getPatientBloodAndEcoPanel() {
		JPanel dataPanel = null;
		dataPanel = new JPanel(new java.awt.GridLayout(1, 2));
		dataPanel.add(setMyBorder(getPatientBloodTypePanel(),
				MessageBundle.getMessage("angal.admission.bloodtype")));
		dataPanel.add(setMyBorder(getPatientEcoStatusPanel(),
				MessageBundle.getMessage("angal.admission.insurance")));
		return dataPanel;
	}
	...
	private JPanel getPatientKinAndTelephonePanel() {
		JPanel dataPanel = null;
		dataPanel = new JPanel(new java.awt.GridLayout(1, 2));
		dataPanel.add(setMyBorder(getPatientKinPanel(),
				MessageBundle.getMessage("angal.admission.nextkin")));
		dataPanel.add(setMyBorder(getPatientTelephonePanel(),
				MessageBundle.getMessage("angal.admission.telephone")));
		return dataPanel;
	}
	...
	private JPanel getPatientTitlePanel() {
		JLabel l = new JLabel(
				MessageBundle.getMessage("angal.admission.patientsummary")
						+ " (" + MessageBundle.getMessage("angal.patient.code")
						+ ": " + patient.getCode() + ")");
		l.setBackground(Color.CYAN);
		JPanel lP = new JPanel(new FlowLayout(FlowLayout.CENTER, insetSize,
				insetSize));
		lP.add(l);
		return lP;
	}
	...
	private JPanel getPatientCard() {
		...
		cardPanel.setBorder(BorderFactory.createEmptyBorder(insetSize,
		...
		patientData.setBorder(BorderFactory.createEmptyBorder(insetSize,
				insetSize, insetSize, insetSize));
		...
		JLabel patientName = new JLabel(
				MessageBundle.getMessage("angal.patient.name") + ": "
						+ filtra(patient.getName()));
		JLabel patientAge = new JLabel(
				MessageBundle.getMessage("angal.patient.age") + ": "
						+ TimeTools.getFormattedAge(patient.getBirthDate()));
		JLabel patientSex = new JLabel(
				MessageBundle.getMessage("angal.patient.sex") + ": "
						+ patient.getSex());
		JLabel patientTOB = new JLabel(
				MessageBundle.getMessage("angal.patient.tobm") + ": "
						+ filtra(patient.getBloodType()));

		//JLabel patientPhoto = new JLabel();
		//Image photo = null;
		/*try {
			if(patient!=null && patient.getCode()!=null){
				String fileName=PatientBrowserManager.getPatientPhotoPath(patient.getCode());
				String chemin=Param.string("PHOTOSDIR")+File.separator+fileName;
				File file=new File(chemin);
				if(file.exists()){
					photo = ImageIO.read(new File(chemin));
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
		...
		/*if (photo != null) {
			patientPhoto
					.setIcon(new ImageIcon(scaleImage(imageMaxWidth, photo)));
		} else {
			try {
				Image noPhotoImage = ImageIO.read(new File(
						"rsc/images/nophoto.png"));
				patientPhoto.setIcon(new ImageIcon(scaleImage(imageMaxWidth,
						noPhotoImage)));
	...
	private JPanel getPatientNamePanel() {
		JLabel l = new JLabel(patient.getSecondName() + " "
				+ patient.getFirstName());
		JPanel lP = new JPanel(new FlowLayout(FlowLayout.LEFT, insetSize,
				insetSize));
		lP.add(l);
		return lP;
	}
	...
	private JPanel getPatientKinPanel() {
		JLabel l = null;
		if (patient.getNextKin() == null
				|| patient.getNextKin().equalsIgnoreCase("")) {
			// l = new
			// JLabel(MessageBundle.getMessage("angal.admission.unknown"));
			l = new JLabel(" ");
		} else {
	...
	private JPanel getPatientTelephonePanel() {
		...
		if (patient.getTelephone() == null
				|| patient.getTelephone().equalsIgnoreCase("")) {
			// l = new
			// JLabel(MessageBundle.getMessage("angal.admission.unknown"));
			l = new JLabel(" ");
		} else {
	...
	private JPanel getPatientAddressPanel() {
		...
		if (patient.getAddress() == null
				|| patient.getAddress().equalsIgnoreCase("")) {
			// l = new
			// JLabel(MessageBundle.getMessage("angal.admission.unknown"));
			l = new JLabel(" ");
		} else {
	...
	private JPanel getPatientBloodTypePanel() {
		...
		if (c == null || c.equalsIgnoreCase("Unknown")) {
			l = new JLabel(
					MessageBundle
							.getMessage("angal.admission.bloodtypeisunknown"));
			l = new JLabel(" ");
		} else {
			// l = new
	...
	private JPanel getPatientEcoStatusPanel() {
		JLabel l = null;
		char c = patient.getHasInsurance().charAt(0);
		if (c == 'Y') {
			l = new JLabel(
					MessageBundle.getMessage("angal.admission.hasinsuranceyes"));
		} else if (c == 'N') {
			l = new JLabel(
					MessageBundle.getMessage("angal.admission.hasinsuranceno"));
		} else {
	...
	private JPanel getPatientSexPanel() {
		JLabel l = new JLabel(
				(patient.getSex().equals(new Character('F')) ? MessageBundle
						.getMessage("angal.admission.female") : MessageBundle
						.getMessage("angal.admission.male")));
		JPanel lP = new JPanel(new FlowLayout(FlowLayout.LEFT, insetSize,
				insetSize));
		lP.add(l);
		return lP;
	...
	private JPanel getPatientParentNewsPanel() {
		...
		if (patient.getMother().charAt(0) == 'A')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.motherisalive"));
		else if (patient.getMother().charAt(0) == 'D')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.motherisdead"));
		;
		// added
		labelBfr.append((patient.getMother_name() == null
				|| patient.getMother_name().compareTo("") == 0 ? "<BR>" : "("
				+ patient.getMother_name() + ")<BR>"));
		if (patient.getFather().charAt(0) == 'A')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.fatherisalive"));
		else if (patient.getFather().charAt(0) == 'D')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.fatherisdead"));
		// added
		labelBfr.append((patient.getFather_name() == null
				|| patient.getFather_name().compareTo("") == 0 ? "<BR>" : "("
				+ patient.getFather_name() + ")<BR>"));
		if (patient.getParentTogether().charAt(0) == 'Y')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.parentslivetoghether"));
		else if (patient.getParentTogether().charAt(0) == 'N')
			labelBfr.append(MessageBundle
					.getMessage("angal.admission.parentsnotlivingtogether"));
		else
			labelBfr.append("<BR>");
