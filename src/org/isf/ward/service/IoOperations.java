...
public class IoOperations {
	...
	public ArrayList<Ward> getWards(String wardID) throws OHException {
		...
		StringBuilder sqlString = new StringBuilder("SELECT * FROM WARD ORDER BY WRD_NAME");
	...
	public Ward getWard(String wardID) throws OHException {
		...
		if (wardID != null && wardID.trim().length() > 0) {
			sqlString.append(" WHERE WRD_ID_A = ?");
			params.add(wardID);
		}		
		DbQueryLogger dbQuery = new DbQueryLogger();
		try {
	...
	public boolean deleteWard(String wardID) throws OHException{
		String string = "DELETE FROM WARD WHERE WRD_ID_A = ?";
		List<Object> params = Collections.<Object>singletonList(wardID);
		DbQueryLogger dbQuery = new DbQueryLogger();
		boolean result = false;
		try {
			result = dbQuery.setDataWithParams(string,params,true);
	...
	public HashMap<String, Integer> getCountAllBeds() throws OHException, ParseException{
		String query1 = "SELECT SUM(WRD_NBEDS) as TOTAL  from ward ";
        HashMap<String, Integer> results = new HashMap<String, Integer>();
		DbQueryLogger dbQuery = new DbQueryLogger();
		try {
			ResultSet resultSet = dbQuery.getData(query1, true);
			if (resultSet.next()) {
				results.put("totalbeds", resultSet.getInt("TOTAL"));
			}			
		}catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		}
