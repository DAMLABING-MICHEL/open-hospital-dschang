...
public class PatVacManager {
	 ...
	 * @return the list of {@link PatientVaccine}s
	 */
	public ArrayList<PatientVaccine> getPatientVaccine(String vaccineTypeCode,String vaccineCode, String patientCode, 
			GregorianCalendar dateFrom, GregorianCalendar dateTo, 
			char sex, int ageFrom, int ageTo) {
		try {
			return ioOperations.getPatientVaccine(vaccineTypeCode, vaccineCode, patientCode, dateFrom, dateTo, sex, ageFrom, ageTo);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
		}
	...
	public boolean deletePatientVaccine(int patVacID) {
		try {
			return ioOperations.deletePatientVaccine(patVacID);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return false;
		}
	...
	public HashMap<String, Integer> getCountSpecificVacc(int year, int idVacc){	
		try {
			return ioOperations.getCountSpecificVacc(year,idVacc);
		} catch (OHException e) {
			JOptionPane.showMessageDialog(null, e.getMessage());
			return null;
