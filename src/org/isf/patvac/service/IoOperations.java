...
public class IoOperations {
	 ...
	 * 
	 * @param minusOneWeek
	 *            - if <code>true</code> return the last week
	 * @return the list of {@link PatientVaccine}s
	 * @throws OHException
	 */
	public ArrayList<PatientVaccine> getPatientVaccine(boolean minusOneWeek)
			throws OHException {
		GregorianCalendar timeTo = TimeTools.getServerDateTime();
		GregorianCalendar timeFrom = TimeTools.getServerDateTime();
		if (minusOneWeek)
	 ...
	 * @throws OHException
	 */
	public ArrayList<PatientVaccine> getPatientVaccine(String vaccineTypeCode,
			String vaccineCode, GregorianCalendar dateFrom,
		...
		sqlString
				.append("SELECT PV.*, V.*, P.PAT_SNAME, P.PAT_FNAME, P.PAT_AGE, P.PAT_SEX , VT.VACT_ID_A, VT.VACT_DESC"
		...
		try {
			...
			while (resultSet.next()) {
				PatientVaccine pv = new PatientVaccine(
						...
						convertToGregorianDate((Date) resultSet
								.getObject("PAV_DATE")),
						resultSet.getInt("PAV_PAT_ID"), new Vaccine(
								resultSet.getString("VAC_ID_A"),
								resultSet.getString("VAC_DESC"),
								new VaccineType(resultSet
										.getString("VACT_ID_A"), resultSet
										.getString("VACT_DESC")),
								resultSet.getInt("VAC_LOCK")),
						resultSet.getInt("PAV_LOCK"),
						resultSet.getString("PAT_SNAME") + " "
								+ resultSet.getString("PAT_FNAME"),
	 ...
	 */
	public ArrayList<PatientVaccine> getPatientVaccine(String vaccineTypeCode,
			String vaccineCode, String patientCode, GregorianCalendar dateFrom,
			GregorianCalendar dateTo, char sex, int ageFrom, int ageTo)
			throws OHException {
		ArrayList<PatientVaccine> patVacList = null;
		StringBuilder sqlString = new StringBuilder();
		...
		if (patientCode != null) {
			...
			sqlString.append(" AND PAV_PAT_ID = ?");
	 ...
	 * 
	 * @param patVac
	 *            - the {@link PatientVaccine} to insert
	...
	public boolean newPatientVaccine(PatientVaccine patVac) throws OHException {
		...
		try {
			String sqlString = "INSERT INTO PATIENTVACCINE"
					+ " (PAV_YPROG, PAV_DATE, PAV_PAT_ID, PAV_VAC_ID_A,PAV_LOCK, PAV_CREATE_BY, PAV_CREATE_DATE) VALUES"
					+ " (?, ?, ?, ?, ?, ?, ?)";

			parameters.add(patVac.getProgr());
			parameters.add(new java.sql.Timestamp(patVac.getVaccineDate()
					.getTime().getTime()));
			...
			ResultSet res = dbQuery.setDataReturnGeneratedKeyWithParams(
					sqlString, parameters, true);
			if (res.first()) {
	 ...
	 * 
	 * @param patVac
	 *            - the {@link PatientVaccine} to update
	...
	public boolean updatePatientVaccine(PatientVaccine patVac)
		...
		boolean result = false;
		try {
			String sqlString = "UPDATE PATIENTVACCINE SET" + " PAV_YPROG = ?,"
					+ " PAV_DATE = ?," + " PAV_PAT_ID = ?,"
					+ " PAV_VAC_ID_A = ?," + " PAV_LOCK = ?, PAV_MODIFY_BY= ?, PAV_MODIFY_DATE= ? "
					+ " WHERE PAV_ID = ? ";
	 ...
	 * 
	 * @param patVac
	 *            - the {@link PatientVaccine} to delete
	...
	public boolean deletePatientVaccine(PatientVaccine patVac)
			throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object> singletonList(patVac
				.getCode());
		boolean result = false;
		try {
	...
	public boolean deletePatientVaccine(int patVacID) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object> singletonList(patVacID);
		boolean result = false;
		try {
			String sqlString = "DELETE FROM PATIENTVACCINE WHERE PAV_ID = ?";
			result = dbQuery.setDataWithParams(sqlString, parameters, true);
	...
	public int getProgYear(int year) throws OHException {
		...
		ResultSet resultSet;
		StringBuilder sqlString = new StringBuilder(
				"SELECT MAX(PAV_YPROG) FROM PATIENTVACCINE");
		...
		if (year == 0) {
			...
			resultSet = dbQuery.getDataWithParams(sqlString.toString(),
					parameters, true);
		}
	...
	public HashMap<String, Integer> getCountSpecificVacc(int year, int idVacc) throws OHException, ParseException{
		String query1 = "SELECT count(PAV_ID) as TOTAL  from patientvaccine where  PAV_DATE BETWEEN ? AND ? and PAV_VAC_ID_A=?";
		...
		try {
			...
			if (resultSet.next()) {
				results.put("totalVcc",resultSet.getInt("TOTAL"));
			}
