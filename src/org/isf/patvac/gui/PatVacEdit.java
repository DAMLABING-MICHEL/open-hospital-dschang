...
//public class PatVacEdit extends ModalJFrame
public class PatVacEdit extends JDialog  {
	...
	public PatVacEdit(JFrame myFrameIn, PatientVaccine patientVaccineIn, Patient patient, boolean action) {
		super(myFrameIn, true);
		//super();
		insert = action;
		patVac = patientVaccineIn;
	...
	private JPanel getDataPanel() {
		if (dataPanel == null) {
			...
			vaccineDateFieldCal.setLocale(new Locale(Param.string("LANGUAGE")));
	...
	private JComboBox getPatientComboBox(String regExp) {
		...
		if (patSelected != null) {
			...
			if (patientComboBox.getItemCount() > 0 && Param.bool("ENHANCEDSEARCH")) {
				if (patientComboBox.getItemAt(0) instanceof Patient) {
					selectedPatient = (Patient) patientComboBox.getItemAt(0);
					setPatient(selectedPatient);
				} else
