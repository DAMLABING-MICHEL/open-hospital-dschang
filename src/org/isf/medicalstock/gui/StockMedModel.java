...
public class StockMedModel extends DefaultTableModel {
	...
	public void filter(String searchValue){
		...
		for (Iterator<Medical> iterator = initList.iterator(); iterator.hasNext();) {
			...
			if(med.getProd_code().trim().equalsIgnoreCase(searchValue.trim())){
				medList.add(med);
			}
			else if(NormalizeString.normalizeContains(med.getProd_code().toLowerCase().trim()+med.getDescription().toLowerCase(), searchValue.toLowerCase().trim())){
				medList.add(med);
			}
		}
	}
