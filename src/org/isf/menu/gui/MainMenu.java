...
public class MainMenu extends JFrame implements ActionListener, Login.LoginListener, SubMenu.CommandListener {
	...
	public static String getUserWard() {
		return myUser.getWardCode();
	}
	...
	//enable mortuary module
	private boolean enableMortuary;
	...
	public MainMenu() {
		...
		try {
			...
			internalPharmacies = Param.bool("INTERNALPHARMACIES") ;   
			debug = Param.bool("DEBUG");   
			if (debug) {
				logger.info("Debug: OpenHospital in debug mode.");
			}
			
			flag_Xmpp = Param.bool("XMPPMODULEENABLED"); 
			flag_Sms = Param.bool("SMSENABLED"); 
			enableMortuary = Param.bool("ENABLEMORTUARYMODULE");
		...
		//if enable mortuary module is false then remove
		if(!enableMortuary) {
			ArrayList<UserMenuItem> junkMenu = new ArrayList<UserMenuItem>();
			Iterator<UserMenuItem> it = myMenu.iterator();
			while (it.hasNext()) {
				UserMenuItem umi = it.next();
				if (umi.getCode().equalsIgnoreCase("MORTUARY"))
					junkMenu.add(umi);
				else if (umi.getCode().equalsIgnoreCase("deathreason"))
					junkMenu.add(umi);
				else if (umi.getCode().equalsIgnoreCase("mortuaryprice"))
					junkMenu.add(umi);
			}
			Iterator<UserMenuItem> altIt = junkMenu.iterator();
			while (altIt.hasNext()) {
		...
		myFrame.setAlwaysOnTop(Param.bool("MAINMENUALWAYSONTOP"));
		...
		for (UserMenuItem u : myMenu) {
			if (u.getCode().equals(itemMenuCode)) {
				if (u.getCode().equalsIgnoreCase("EXIT")) {
					...
					//System.out.println("is a sub menu");
					...
					try {
						...
						try {
							//System.out.println("creattion of menu");
							if(Param.bool("WITHMODALWINDOW")){
								((ModalJFrame) target).showAsModal(this);
							}else{
								((ModalJFrame) target).show(this);
							}							
