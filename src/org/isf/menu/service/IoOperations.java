...
public class IoOperations {
		...
		try {
			...
			while (resultSet.next()) {
				users.add(new User(resultSet.getString("US_ID_A"), 
						...
						resultSet.getString("US_WRD_ID_A"),
						
						resultSet.getString("US_FIRSTNAME"), 
						resultSet.getString("US_SURNAME"),
						resultSet.getString("US_PHONE")
	...
	public String getUsrFullName(String userName) throws OHException {
		String string = "SELECT CONCAT(US_FIRSTNAME, ' ', US_SURNAME)  as fullname  FROM USER WHERE US_ID_A = ?";
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object>singletonList(userName);
		String info = null;
		try{
	...
	public String getUsrName(String userName) throws OHException {
		String string = "SELECT US_FIRSTNAME, US_SURNAME FROM USER WHERE US_ID_A = ?";
		DbQueryLogger dbQuery = new DbQueryLogger();
		List<Object> parameters = Collections.<Object>singletonList(userName);
		String name = null;
		try{
			ResultSet result =dbQuery.getDataWithParams(string, parameters, true);
			while(result.next()) name = result.getString("US_FIRSTNAME")+" "+result.getString("US_SURNAME");
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		} finally{
			dbQuery.releaseConnection();
	...
	public boolean newUser(User user) throws OHException {
		...
		String sqlString = "INSERT INTO USER (US_ID_A , US_UG_ID_A, US_PASSWD, US_DESC, US_WRD_ID_A, US_FIRSTNAME,"
				+ " US_SURNAME, US_PHONE, US_CREATE_BY, US_CREATE_DATE ) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?)";
		parameters.add(user.getUserName());
		parameters.add(user.getUserGroupName());
		parameters.add(user.getPasswd());
		parameters.add(user.getDesc());
		parameters.add(user.getWardCode());
	...
	public boolean updateUser(User user) throws OHException {
		...
		String sqlstring = "UPDATE USER SET US_DESC = ?, US_WRD_ID_A = ?    , US_FIRSTNAME = ?  , US_SURNAME = ?  , US_PHONE = ?"
				+ ",US_MODIFY_BY= ?, US_MODIFY_DATE= ?  WHERE US_ID_A = ?";
		parameters.add(user.getDesc());
		parameters.add(user.getWardCode());
	...
	public boolean updatePassword(User user) throws OHException {
		...
		String sqlstring = "UPDATE USER SET US_PASSWD = ? ,US_MODIFY_BY= ?, US_MODIFY_DATE= ?WHERE US_ID_A = ?";
		parameters.add(user.getPasswd());
		parameters.add(MainMenu.getUser());
		parameters.add(new java.sql.Timestamp(TimeTools.getServerDateTime().getTime().getTime()));
		parameters.add(user.getUserName());
