...
public class IoOperations {
	...
	public Hospital getHospital() throws OHException {
		...
		try {	
			...
			if (resultSet.first()) {
				...
				pHospital.setPopulation_area(resultSet.getInt("HOS_POPULATION_AREA"));
			}
	...
	public boolean updateHospital(Hospital hospital) throws OHException {
		...
		params.add(sanitize(hospital.getPopulation_area()+""));
	...
	public HashMap<String, Integer> getPopulationArea() throws OHException, ParseException{
		String query1 = "SELECT HOS_POPULATION_AREA as TOTAL  from hospital ";
        HashMap<String, Integer> results = new HashMap<String, Integer>();
		DbQueryLogger dbQuery = new DbQueryLogger();
		try {
			ResultSet resultSet = dbQuery.getData(query1, true);
			if (resultSet.next()) {
				results.put("totalpopulation", resultSet.getInt("TOTAL"));
			}			
		}catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
		}
