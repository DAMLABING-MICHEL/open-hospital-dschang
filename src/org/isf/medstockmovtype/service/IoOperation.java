...
public class IoOperation {
	...
	public boolean deleteMedicaldsrstockmovType(String medicaldsrstockmovTypeID) throws OHException {
		
		List<Object> parameters = Collections.<Object>singletonList(medicaldsrstockmovTypeID);
		String query = "delete from MEDICALDSRSTOCKMOVTYPE where MMVT_ID_A = ?";
		DbQueryLogger dbQuery = new DbQueryLogger();
		boolean result = false;
		try {
	 ...
	 * @throws OHException if an error occurs during the check.
	 */
	public MovementType getMovementType(String code) throws OHException {
		DbQueryLogger dbQuery = new DbQueryLogger();
		try{
			List<Object> parameters = Collections.<Object>singletonList(code);
			String query = "SELECT * FROM MEDICALDSRSTOCKMOVTYPE where MMVT_ID_A = ?";
			ResultSet set = dbQuery.getDataWithParams(query, parameters, true);
			if(set.next()){
					MovementType movementType = new MovementType();
					movementType.setCode(set.getString("MMVT_ID_A"));
					movementType.setDescription(set.getString("MMVT_DESC"));
					movementType.setType(set.getString("MMVT_TYPE"));
					return movementType;
			}
		} catch (SQLException e) {
			throw new OHException(MessageBundle.getMessage("angal.sql.problemsoccurredwiththesqlistruction"), e);
