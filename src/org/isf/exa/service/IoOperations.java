...
public class IoOperations {
	...
	/**
	 * Returns a list of {@link ExamRow}s that matches passed exam code and
	 * description
	 * 
	 * @param aExamCode
	 *            - the exam code
	...
	public ArrayList<ExamRow> getExamRow(String aExamCode, String aDescription)
		...
		try {
			ResultSet resultSet = dbQuery.getDataWithParams(query.toString(),
					params, true);
			pRow = new ArrayList<ExamRow>(resultSet.getFetchSize());
			while (resultSet.next()) {
				pRow.add(new ExamRow(resultSet.getString("EXR_ID"), resultSet
						.getString("EXR_EXA_ID_A"), resultSet
						.getString("EXR_DESC")));
			}
	 ...
	 * @throws OHException
	 */
	public ArrayList<Exam> getExamsByDesc(String description)
			throws OHException {
		...
		StringBuilder query = new StringBuilder(
				"SELECT * FROM EXAM JOIN EXAMTYPE ON EXA_EXC_ID_A = EXC_ID_A");
		if (description != null) {
			query.append(" WHERE EXC_DESC LIKE ?");
			params.add('%' + description + '%');
		}
		query.append(" ORDER BY EXC_DESC, EXA_DESC ASC");
		//query.append(" ORDER BY EXA_DESC ASC");

		try {
			ResultSet resultSet = null;
			if (description != null)
				resultSet = dbQuery.getDataWithParams(query.toString(), params,
						true);
			else
				resultSet = dbQuery.getData(query.toString(), true);
			pExams = new ArrayList<Exam>(resultSet.getFetchSize());
			while (resultSet.next()) {
				pExams.add(new Exam(resultSet.getString("EXA_ID_A"), resultSet
						.getString("EXA_DESC"),
						new ExamType(resultSet.getString("EXC_ID_A"), resultSet
	...
	public ArrayList<Exam> getExamsOrderedByDesc()
			throws OHException {
		ArrayList<Exam> pExams = null;
		DbQueryLogger dbQuery = new DbQueryLogger();
	...
	/**
	 * Returns an {@link Exam}s that matches passed code
	 * 
	 * @param code
	 *            - the exam code
	 * @return the exam {@link Exam} matching the code or null if not found
	 * @throws OHException
	 */
	public Exam getExamsByCode(String code) throws OHException {
		Exam exam = null;
		DbQueryLogger dbQuery = new DbQueryLogger();
		ArrayList<Object> params = new ArrayList<Object>(1);
		...
		try {
			...
			if (resultSet.next()) {
				exam = new Exam(resultSet.getString("EXA_ID_A"),
						resultSet.getString("EXA_DESC"), new ExamType(
								resultSet.getString("EXC_ID_A"),
								resultSet.getString("EXC_DESC"),
								resultSet.getString("EXC_ACCOUNT")
								//resultSet.getString("EXC_EXPENSE_ACCOUNT")
								),
						resultSet.getInt("EXA_PROC"),
						resultSet.getString("EXA_DEFAULT"),
						resultSet.getInt("EXA_LOCK"));
			}
		} catch (SQLException e) {
			throw new OHException(
					MessageBundle
	...
	public ArrayList<ExamType> getExamType() throws OHException {
		...
		try {
			...
			while (resultSet.next()) {
				pexamtype.add(new ExamType(resultSet.getString(1), resultSet
						.getString(2)));
			}
		} catch (SQLException e) {
			throw new OHException(
	 ...
	 * 
	 * @param exam
	 *            - the {@link Exam} to insert
	 * @return <code>true</code> if the {@link Exam} has been inserted,
	 ...
	 * 
	 * @param examRow
	 *            - the {@link ExamRow} to insert
	 * @return <code>true</code> if the {@link ExamRow} has been inserted,
	 *         <code>false</code> otherwise
	 * @throws OHException
	 */
	 ...
	 * 
	 * @param exam
	 *            - the {@link Exam} to update
	 * @param check
	 *            - if <code>true</code> it will perform an integrity check
	 * @return <code>true</code> if the {@link Exam} has been updated,
	 *         <code>false</code> otherwise
	 * @throws OHException
	 */
	public boolean updateExam(Exam exam, boolean check) throws OHException {
		...
		try {
			if (check) {
				...
				List<Object> params = Collections.<Object> singletonList(exam
						.getCode());
				...
				if (set.first()) {
					...
					throw new OHException(
							MessageBundle
									.getMessage("angal.sql.couldntfindthedataithasprobablybeendeleted"));
				}
			}

			String query = "UPDATE EXAM SET EXA_DESC = ?, EXA_LOCK = ?, EXA_PROC = ?, EXA_DEFAULT = ?, EXA_EXC_ID_A = ? WHERE EXA_ID_A = ?";
			ArrayList<Object> params = new ArrayList<Object>();
			...
			params.add(exam.getProcedure());
			params.add(sanitize(exam.getDefaultResult()));
			params.add(sanitize(exam.getExamtype().getCode()));
			params.add(exam.getCode());
			...
			if (result)
				...
				throw new OHException(
						MessageBundle
								.getMessage("angal.sql.thedataisnomorepresent")); // the
																					// record
																					// has
																					// been
																					// deleted
	 ...
	 * 
	 * @param exam
	 *            - the {@link Exam} to delete
	 * @return <code>true</code> if the {@link Exam} has been deleted,
	 *         <code>false</code> otherwise
	 * @throws OHException
	 */
	public boolean deleteExam(String code) throws OHException {
		String query;
		DbQueryLogger dbQuery = new DbQueryLogger();
		boolean result = true;
		try {
	 ...
	 * 
	 * @param examRow
	 *            - the {@link ExamRow} to delete
	 * @return <code>true</code> if the {@link ExamRow} has been deleted,
	 *         <code>false</code> otherwise
	 * @throws OHException
	 */
	public boolean deleteExamRow(String code) throws OHException {
	 ...
	 * @param the
	 *            {@link Exam}
	 * @return <code>true</code> if the Exam code has already been used,
	 *         <code>false</code> otherwise
	 * @throws OHException
	 */
