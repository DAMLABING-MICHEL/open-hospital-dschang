...
public class IoOperation {
	...
	public ArrayList<MedicalType> getMedicalTypes() throws OHException {
		...
		String query = "SELECT MDSRT_ID_A,MDSRT_DESC, MDSRT_ACCOUNT, MDSRT_EXPENSE_ACCOUNT FROM MEDICALDSRTYPE ORDER BY MDSRT_DESC";
		...
		try{
			...
			while (resultSet.next()) {
				medicaltypes.add(new MedicalType(resultSet.getString("MDSRT_ID_A"), resultSet.getString("MDSRT_DESC"), resultSet.getString("MDSRT_ACCOUNT"), resultSet.getString("MDSRT_EXPENSE_ACCOUNT")));
			}
	...
	public MedicalType getMedicalTypeByCode(String code) throws OHException {
		ArrayList<MedicalType> medicaltypes = null;
		String query = "SELECT MDSRT_ID_A,MDSRT_DESC, MDSRT_ACCOUNT, MDSRT_EXPENSE_ACCOUNT FROM MEDICALDSRTYPE WHERE MDSRT_ID_A=? ORDER BY MDSRT_DESC";
		DbQueryLogger dbQuery = new DbQueryLogger();
		...
		return medicaltypes.size()>0?medicaltypes.get(0):null;
	}
	...
	public boolean updateMedicalType(MedicalType medicalType) throws OHException {
		...
		try {
			...
			parameters.add(medicalType.getAccount());
			parameters.add(medicalType.getExpenseAccount());
			parameters.add(medicalType.getCode());
	...
	public boolean newMedicalType(MedicalType medicalType) throws OHException {
		...
		try {
			...
			parameters.add(medicalType.getAccount());
			parameters.add(medicalType.getExpenseAccount());
			String query = "INSERT INTO MEDICALDSRTYPE (MDSRT_ID_A, MDSRT_DESC, MDSRT_ACCOUNT, MDSRT_EXPENSE_ACCOUNT) VALUES (?,?,?,?)";
			result = dbQuery.setDataWithParams(query, parameters, true);
		} finally{
			dbQuery.releaseConnection();
		}
	...
	public boolean deleteMedicalType(String medicalTypeID) throws OHException {
		
		List<Object> parameters = Collections.<Object>singletonList(medicalTypeID);
		String query = "DELETE FROM MEDICALDSRTYPE WHERE MDSRT_ID_A = ?";
		DbQueryLogger dbQuery = new DbQueryLogger();
		boolean result = false;
		try {
