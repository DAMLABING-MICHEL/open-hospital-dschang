...
public abstract class XMLDocumentManager {
	...
	public XMLDocumentManager() {
		try {
			...
			String path = VideoDevicesManager.resolutionsFilePath;

			URL url = XMLDocumentManager.class.getClassLoader().getResource(
					VideoDevicesManager.resolutionsFilePath);
			if (url != null) {
				path = url.getPath();
			}
	...
	public void writeResolutions(String deviceId, ArrayList<String> resolutions) {
		...
		for (String res : resolutions) {
			...
			System.out.println("[XMLdebug] Adding: <resolution," + os + ","
					+ res + ">");
		}
	...
	private Element writeDeviceElement(String deviceId) {
		...
		// altrimenti se esiste già modifica o aggiungi il valore dell'attributo
		// id
	...
	protected Element getDeviceElement(String deviceId) {
		...
		for (int s = 0; s < deviceNodeList.getLength(); s++) {
			...
			String identificationAttributeName = getIdentificationAttributeName(); // id
																					// for
																					// linux,
																					// description
																					// for
			...
			String currentId = deviceElement
					.getAttribute(identificationAttributeName);

			// System.out.println("[XMLdebug] Ricerca device, id corrente: " +
			// currentId);
	...
	public ArrayList<String> readResolutionsForCurrentOs(String deviceId) {
		...
		Element deviceElement = getDeviceElement(deviceId);
		if (deviceElement != null) {
			NodeList resolutionNodeList = deviceElement
	...
	private Element getDefaultResolution(NodeList resolutionNodeList) {
		...
		for (int i = 0; i < resolutionNodeList.getLength(); i++) {
			...
			if (osAttrValue.equals(os) || osAttrValue.equals("all")) {
				Node defaultAttribute = resTagElement.getAttributes()
	...
	private Element getOsResolutionElement(int width, int height,
			NodeList resolutionNodeList) {
		String os = getOsAttributeValue();
	...
	public void removeResolutionsForCurrentOs(String deviceId) {
		...
		for (int i = 0; i < nres; i++) {
			...
			if (attrValue.equals(os)) {
				...
				System.out.println("[XMLdebug] Removing: <os, " + attrValue
						+ "," + resTagElement.getTextContent() + ">");
				deletedNodes++;
			} else if (attrValue.equals("all")) {
				String otherOs = (os == "win") ? "linux" : "win";
				((Element) resTagElement).setAttribute("os", otherOs);
				System.out.println("[XMLdebug] Changing: <os, " + attrValue
						+ "," + resTagElement.getTextContent() + "> to "
						+ otherOs);
			}
		}
	...
	public boolean setDefaultResolutionForDevice(int width, int height,
			String deviceId) {
		Element deviceElement = getDeviceElement(deviceId);
		NodeList resolutionNodeList = deviceElement
				.getElementsByTagName("Resolution");
		...
		Element resolutionElement = getOsResolutionElement(width, height,
				resolutionNodeList);
		if (resolutionElement != null) {
			resolutionElement.setAttribute("default", "true");
			saveFile();
			System.out.println("Set default resolution: " + width + "x"
					+ height);
			return true;
		}

		System.out.println("Trying to set default resolution: " + width + "x"
				+ height + " (not found)");
		return false;
	}
	...
	protected void saveFile() {
		...
		File file = new File(path);
		if (!file.exists()) {
			file = new File("rsc/" + VideoDevicesManager.resolutionsFilePath);
		}
		try {
