...
public class LabEdit extends JDialog  {
	...
	private void fireLabUpdated() {
		...
		EventListener[] listeners = labEditListener
				.getListeners(LabEditListener.class);
	...
	private void initialize() {
		...
		this.setBounds(30, 30, panelWidth + 20, dataPanelHeight
				+ resultPanelHeight + buttonPanelHeight + 30);
		this.setContentPane(getJContentPane());
		this.setResizable(false);
		if (insert) {
			this.setTitle(MessageBundle
					.getMessage("angal.lab.newlaboratoryexam")
					+ "("
					+ VERSION
					+ ")");
		} else {
			this.setTitle(MessageBundle
					.getMessage("angal.lab.editlaboratoryexam")
					+ "("
					+ VERSION
					+ ")");
		}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			resultPanel.setBounds(0, dataPanelHeight, panelWidth,
					resultPanelHeight);
			if (!insert) {
	...
	private JPanel getDataPanel() {
		if (dataPanel == null) {
			...
			// material
			matLabel = new JLabel(
					MessageBundle.getMessage("angal.lab.material"));
	...
	private JCheckBox getInPatientCheckBox() {
		if (inPatientCheckBox == null) {
			inPatientCheckBox = new JCheckBox(
					MessageBundle.getMessage("angal.lab.in"));
			if (!insert)
				inPatientCheckBox.setSelected(lab.getInOutPatient()
						.equalsIgnoreCase("I"));
	...
	private JComboBox getPatientComboBox() {
		if (patientComboBox == null) {
			...
			patientComboBox.addItem(MessageBundle
					.getMessage("angal.lab.selectapatient"));
			...
			patientComboBox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if (patientComboBox.getSelectedIndex() > 0) {
						...
						Patient pat = (Patient) patientComboBox
								.getSelectedItem();
						patTextField.setText(pat.getName());
						ageTextField.setText(pat.getAge() + "");
						sexTextField.setText(pat.getSex() + "");
						inPatientCheckBox.setSelected(admMan
								.getCurrentAdmission(pat) != null ? true
								: false);
					}
				}
			});
	...
	private JPanel getButtonPanel() {
		if (buttonPanel == null) {
			...
			buttonPanel.setBounds(0, dataPanelHeight + resultPanelHeight,
	...
	private JComboBox getExamComboBox() {
		if (examComboBox == null) {
			...
			examComboBox.addItem(MessageBundle
					.getMessage("angal.lab.selectanexam"));
			for (Exam elem : exams) {
				if (!insert && elem.getCode() != null) {
					if (elem.getCode().equalsIgnoreCase(
							(lab.getExam().getCode()))) {
						examSel = elem;
					}
				}
...
//			matComboBox.addItem(MessageBundle.getMessage("angal.lab.tissues"));
	...
	private JTextArea getNoteTextArea() {
		if (noteTextArea == null) {
			...
			noteTextArea.setBorder(BorderFactory
	...
	private JButton getCancelButton() {
		if (cancelButton == null) {
			...
			cancelButton.setText(MessageBundle
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					if (examComboBox.getSelectedIndex() == 0) {
						JOptionPane.showMessageDialog(null, MessageBundle
								.getMessage("angal.lab.pleaseselectanexam"));
						return;
					}
					// materiale
					if (matComboBox.getSelectedIndex() == 0) {
						JOptionPane.showMessageDialog(null, MessageBundle
								.getMessage("angal.lab.pleaseselectamaterial"));
						return;
					}
					...
					try {
						...
						JOptionPane.showMessageDialog(
								null,
								MessageBundle
										.getMessage("angal.lab.pleaseinsertavalidexamdate"));
					...
					Integer patId = -1;
					if (patientComboBox.getSelectedIndex() > 0)
						patId = ((Patient) (patientComboBox.getSelectedItem()))
								.getCode();
					...
					if (!(sex.equals("M") || sex.equals("F"))) {
						JOptionPane.showMessageDialog(
								null,
								MessageBundle
										.getMessage("angal.lab.pleaseinsertmformaleorfforfemale"));
						return;
					}
					...
					lab.setInOutPatient((inPatientCheckBox.isSelected() ? "I"
							: "R"));
					lab.setPatId(patId);
					...
					if (examSelected.getProcedure() == 1)
						...
						lab.setResult(MessageBundle
								.getMessage("angal.lab.multipleresults"));
						for (int i = 0; i < resultPanel.getComponentCount(); i++) {
							...
							if (subPanel
								...
								LaboratoryRow labRowObject=new LaboratoryRow(null, lab.getCode(), eRows.get(i).getDescription(), subPanel.getResultValue());
					...
					if (insert) {
						...
						if (examSelected.getProcedure() == 1)
							...
							result = manager
									.editLabSecondProcedure(lab, labRow);
					}

					if (!result)
						JOptionPane.showMessageDialog(null, MessageBundle
								.getMessage("angal.lab.thedatacouldnotbesaved"));
					else {
						fireLabUpdated();
						dispose();
					}
